...
public OnFilterScriptInit()
{
	...
	print(" ");
	print("Esta é a nova versão Tablet Windows 8");
	print("for SA-MP 0.3E+");
	print("V4.0");
	print("Criado por Renan");
	print("	");
	print("Version: 4.0");
 	print("CMD: /tablet");
	print(" ");
	print("Confira Nosso FS by Renan");
	print("________________________________________________");
	...
	TabletWin8Start[31] = TextDrawCreate(187.000000, 198.000000, "Correio");
	...
	TabletWin8Start[32] = TextDrawCreate(193.000000, 245.000000, "Contatos");
	...
	TabletWin8Start[33] = TextDrawCreate(193.000000, 291.000000, "Menssagens");
	...
	TabletWin8Start[35] = TextDrawCreate(273.000000, 198.000000, "Relógio");
	...
	TabletWin8Start[36] = TextDrawCreate(273.000000, 246.000000, "Fotos");
	...
	TabletWin8Start[38] = TextDrawCreate(273.000000, 338.000000, "Tempo");
	...
	TabletWin8Start[41] = TextDrawCreate(341.000000, 291.000000, "Esportes");
	...
	TabletWin8Start[42] = TextDrawCreate(339.000000, 338.000000, "Noticias");
	...
	TabletWin8Start[43] = TextDrawCreate(371.000000, 198.000000, "Armazenar");
	...
	TabletWin8Start[46] = TextDrawCreate(440.000000, 245.000000, "Viagens");
	...
	TabletWin8Start[49] = TextDrawCreate(436.000000, 338.000000, "Musica");
	...
	TabletWin8Pag[1] = TextDrawCreate(328.000000, 262.000000, "informações: este filterscript está em construção~n~~n~BY Renan");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_TABLETCHAT)
    {
        if(response)
        {
            if(!strlen(inputtext)) return ShowPlayerDialog(playerid, DIALOG_TABLETCHAT, DIALOG_STYLE_INPUT, "{FF0000}Player ID:", "Write the Player ID (ID)", "Search", "Cancel");
            if(IsNumeric(inputtext))
            {
				new id = strval(inputtext);
				if(id == playerid)
				{
					ShowPlayerDialog(playerid, DIALOG_TABLETCHAT, DIALOG_STYLE_INPUT, "{FF0000}Speak with yourself? no!         .", "Write the Player ID (ID)", "Search", "Cancel");
				}
    			else
				{
		            if(IsPlayerConnected(id))
		            {
			            ...
			            ShowPlayerDialog(id, DIALOG_TABLETCHAT+1, DIALOG_STYLE_MSGBOX, "{FF0000}Chat invitation", str, "Accept", "Reject");
					...
					{
                		ShowPlayerDialog(playerid, DIALOG_TABLETCHAT, DIALOG_STYLE_INPUT, "{FF0000}Error: Player ID is invalid", "Write the player ID (ID)           .", "Search", "Cancel");
					}
				}
			}
			else
			{
			    ShowPlayerDialog(playerid, DIALOG_TABLETCHAT, DIALOG_STYLE_INPUT, "{FF0000}Write the Player ID", "Write Player ID (ID)", "Search", "Cancel");
			}
		}
	}
    if(dialogid == DIALOG_TABLETCHAT+1)
    {
        if(response)
        {
	        ...
	        ShowPlayerDialog(playerid, DIALOG_TABLETCHAT+2, DIALOG_STYLE_INPUT, str, "Write here your message:", "Send", "Exit");
	        }
        }
        else
        {
			...
			ShowPlayerDialog(GetPVarInt(playerid,"playeronid"), DIALOG_TABLETCHAT+3, DIALOG_STYLE_MSGBOX, "{FF0000}Chat sesion", str,"Exit", "");
        }
	}
    if(dialogid == DIALOG_TABLETCHAT+2)
    {
        if(response)
        {
			...
			if(chatid[playerid] == 1)
			{
		        ...
		        GetPVarString(playerid, "sendername",asd,24);
		        format(str,sizeof(str),"{FF0000}Chat sesion - %s",asd);
		        ShowPlayerDialog(playerid, DIALOG_TABLETCHAT+2, DIALOG_STYLE_INPUT, str, chattext[playerid], "Send", "Exit");
		        new str2[64];
				format(str2,sizeof(str2),"{FF0000}Chat sesion - %s",name);
		        ShowPlayerDialog(GetPVarInt(playerid,"playeronid"), DIALOG_TABLETCHAT+2, DIALOG_STYLE_INPUT, str2, chattext[GetPVarInt(playerid,"playeronid")], "Send", "Exit");
			}
			else
			{
		        new str[64];
		        ...
		        new str2[64];
				format(str2,sizeof(str2),"{FF0000}Chat sesion - %s",name);
		        ShowPlayerDialog(GetPVarInt(playerid,"therplayeronid"), DIALOG_TABLETCHAT+2, DIALOG_STYLE_INPUT, str2, chattext[GetPVarInt(playerid,"therplayeronid")], "Send", "Exit");
			}
		}
		else
		{
			...
			else
			{
				...
				chatid[GetPVarInt(playerid,"therplayeronid")] = 0;
				format(str,sizeof(str),"%s quit the conversation.",name);
				ShowPlayerDialog(GetPVarInt(playerid,"therplayeronid"), DIALOG_TABLETCHAT+3, DIALOG_STYLE_MSGBOX, "{FF0000}Chat sesion", str,"Exit", "");
			}
		}
	}
	return 1;
...
stock ShowPage(playerid,page)
{
	...
	else if(page == 1)PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Contatos");
	else if(page == 2)return PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Mensagens"), ShowPlayerDialog(playerid, DIALOG_TABLETCHAT, DIALOG_STYLE_INPUT, "{FF0000}Write The Player ID", "Put the Player ID (ID)", "Search", "Cancel"),ShowPagForItems(playerid),TextDrawHideForPlayer(playerid,TabletWin8Pag[1]);
	...
	else if(page == 5)return PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Fotos"),ShowPhotosForPlayer(playerid),ShowPagForItems(playerid),TextDrawHideForPlayer(playerid,TabletWin8Pag[1]);
	...
	else if(page == 10)PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Esportes");
	else if(page == 11)PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Noticias");
	else if(page == 12)PlayerTextDrawSetString(playerid,TabletWin8Pag2,"armazenar");
	...
	else if(page == 15)PlayerTextDrawSetString(playerid,TabletWin8Pag2,"Viagens");
...
________________________________________________>                          <________________________________________________
	...
	This the new tablet Windows 8 Version         >                      <
           ...
           Created by Renan                          >                <
													  >              <
            Version: 4.0                               >	Renan   <           SCRIPT FINISH. F5 TO COMPILE
													  >              <
            CMD: /tablet                             >                <
												    >                  <
           Created in Spanish                      >                    <
    and translate to english by Renan             >                      <
                                                 >                        <
________________________________________________>                          <________________________________________________
*/
